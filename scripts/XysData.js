...
function TreeSelected(p, e, t) {
    ...
    if (e.which === 1 || e.button === 0) {
        ...
        var elms = document.querySelectorAll('input[data-name="XysDataEV"]');
        if (elms != null) {
...
function DbElementPop(p, t) {
    var data = [
       { key: 'o', vlu: t }
    ];
    $WaitOn();
    $ApiRequest(p + '/DbElementPop', JSON.stringify(data));
}
function DbElement(p, t) {
    var en = document.getElementById('dbtable');
    ...
    $PopOff();
    $WaitOn();
    $ApiRequest(p + '/DbElement', JSON.stringify(data));
}
